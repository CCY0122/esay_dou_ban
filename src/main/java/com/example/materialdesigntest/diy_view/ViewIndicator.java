...
import com.example.materialdesigntest.util.BaseApplication;
 ...
 * 可自定义一屏显示多少个tab标签（通过设置visibleItemCount）    ps：请不要让tab数少于visibleItemCount。 ps:没写在attrs里
...
public class ViewIndicator extends LinearLayout {
    ...
    public ViewIndicator(Context context, AttributeSet attrs) {
        ...
        paint.setPathEffect(new CornerPathEffect(9)); //圆角
        path = new Path();
...
//                        Log.d("ccy2","onclick");
    ...
    protected void dispatchDraw(Canvas canvas) {
        canvas.save();
        canvas.translate(itemWidth/2 - triangleWidth/2 + triangleTranslate  ,getMeasuredHeight());
...
//        visibleItemCount = 3;
        screenWidth = Mutil.getScreenSize().x;
        itemWidth = (int) (screenWidth / (visibleItemCount));
        triangleWidth = itemWidth / 3 ;
